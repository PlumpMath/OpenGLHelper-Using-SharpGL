...
namespace SharpGLHelper.ModelComponents2
{
    public class Mesh
    {
        ...
        public void BuildMesh(ushort[] indices, vec3[] vertices, vec3[] normals)
        {
        ...
        public void TriangulateMesh(bool calculateNormals, bool updateRawData)
        {
            if (calculateNormals)
                CalculateNormals(); // Normals will invalid after triangulation, so they need to be recalculated.
            if (updateRawData)
                UpdateRawData(); // A lot of mesh data has changed, so the raw verts, indices and normals need to be updated.
            ...
            {
                var newFaces = face.TriangulateFace();
